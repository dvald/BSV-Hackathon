...
This project uses a centralized system for managing theme colors and variables with TypeScript integration.

#### Overview

Previously, colors were hardcoded in multiple places within `theme.css` as hexadecimal values (e.g., `#188b9a`). Changing a color required updating it manually in multiple locations. Now, all colors are centralized in `variables.css` and `theme.css` uses CSS variables, allowing changes from a single source of truth.

#### Architecture

The theme system consists of three main components:

-   **`src/style/variables.css`**: Single source of truth for all color values. All theme colors are defined here as CSS custom properties.
-   **`src/style/theme.css`**: Uses CSS variables from `variables.css`. Contains theme-specific implementations (dark/light themes) that reference the centralized variables.
-   **`src/composables/useTheme.ts`**: Auto-generated TypeScript bindings (DO NOT EDIT MANUALLY). Provides type-safe access to theme variables in JavaScript/TypeScript.
-   **`scripts/generate-theme.cjs`**: Script that automatically generates the TypeScript bindings from the CSS variables.

#### Usage in JavaScript/TypeScript

You can access theme variables in your components using the generated composable:

```ts
import { theme, getThemeVar, useTheme } from "@/composables/useTheme";

// Direct access to a specific variable
const primaryColor = getThemeVar("primary-color");

// Dynamic access using the theme proxy
const dynamicColor = theme["primary-color"];

// Using the composable hook in Vue components
export default defineComponent({
    setup() {
        const { theme, getThemeVar } = useTheme();

        const buttonColor = computed(() => theme["primary-color"]);
...
You can continue using the variables directly in CSS as before:
...
1. **Modify colors** in `src/style/variables.css`:
...
3. **Use the updated variables** in your components with full TypeScript autocomplete and type safety.

#### Benefits

-   **Centralized color management**: Change colors from a single location
-   **TypeScript autocomplete and type safety**: Prevents typos and provides IntelliSense
-   **Automatic synchronization**: CSS and JavaScript always stay in sync
-   **Consistent theming**: Impossible to use undefined color variables
-   **Developer experience**: Clear error messages and helpful console output during generation

#### Available Variables

The following theme variables are currently available (auto-updated when you run `npm run update-theme`):

-   `background-dark`, `background-dark-soft`
-   `background-light`, `background-light-soft`
-   `primary-color`, `secondary-color`, `tertiary-color`
-   `danger-color`, `danger-color-hover`
-   `success-color`, `success-color-hover`
-   `warning-color`, `warning-color-hover`

#### Contrast and Accessibility Considerations

When changing primary and secondary colors, pay special attention to contrast ratios for accessibility and readability. Some parts of the template include specific contrast adjustments marked with special comments:

```css
/*CONTRAST LINE: add to contrast with primary & secondary color */
```

These comments indicate areas where text color is specifically overridden to ensure proper readability against primary/secondary color backgrounds.

**Example from `page-nav-vertical.css`:**

```css
/*CONTRAST LINE: add to contrast with primary & secondary color */
.page-vertical-menu-group-option:hover,
.page-vertical-menu-group-option.current,
.page-vertical-menu-group-option.current:hover {
    color: var(--background-light) !important;
}
```

**When to review these sections:**

-   **Light primary/secondary colors**: If you change to very light colors (e.g., light yellow, light pink), the contrast with white text may be insufficient
-   **Very dark colors**: Ensure sufficient contrast with the chosen text color
-   **Accessibility compliance**: Follow WCAG guidelines for color contrast ratios

**How to identify contrast issues:**

1. Search for `/*CONTRAST LINE:` comments in your CSS files
2. Test the interface with your new colors
3. Use browser dev tools or accessibility checkers to verify contrast ratios
4. Adjust the contrast rules as needed for your specific color palette
