...
import { toTypescriptFileName } from "./utils/text";
...
function generateWrapper(className: string, abi: ABILike, docs?: ContractDoc) {
    ...
    const overloadedMethodsIndex: { [key: string]: number } = Object.create(null);
...
function makeViewFunction(entry: JsonFragment, result: WrapperResult, tabSpaces: string, isOverloaded: boolean, overloadedMethodsIndex: { [key: string]: number }, docs?: ContractDoc) {
    const lines = [];
...
function makeTransactionFunction(entry: JsonFragment, result: WrapperResult, tabSpaces: string, className: string, isOverloaded: boolean, overloadedMethodsIndex: { [key: string]: number }, docs?: ContractDoc) {
    const lines = [];
