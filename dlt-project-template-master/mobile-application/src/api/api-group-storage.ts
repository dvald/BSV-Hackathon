...
import { getApiUrl, generateURIQuery, UploadableFile, appendFileToForm } from "./utils";
...
export class ApiStorage {
    ...
    public static GetStorageConfig(): RequestParams<BsvStorageConfigResponse, CommonErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/api/v1/storage/config` + generateURIQuery({ _time: Date.now() })),
    ...
    public static GetStorageBuckets(): RequestParams<string[], CommonErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/api/v1/storage/buckets` + generateURIQuery({ _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(500, "*", "serverError" in handler ? handler.serverError : handler.temporalError)
                    .add("*", "*", "networkError" in handler ? handler.networkError : handler.temporalError)
    ...
    public static UploadFile(formParams: UploadFileFormParameters): RequestParams<BsvStorageUploadResponse, UploadFileErrorHandler> {
        ...
        appendFileToForm(formData, "file", formParams.file);
        formParams.bucket !== undefined && appendFileToForm(formData, "bucket", formParams.bucket);
        formParams.isPublic !== undefined && appendFileToForm(formData, "isPublic", formParams.isPublic);

        return {
            method: "POST",
            url: getApiUrl(`/api/v1/storage/upload` + generateURIQuery({ _time: Date.now() })),
            form: formData,
            handleError: (err, handler) => {
                new RequestErrorHandler()
    ...
    public static DownloadFile(uhrpUrl: string, queryParams: DownloadFileQueryParameters): RequestParams<BsvStorageDownloadResponse, DownloadFileErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/api/v1/storage/download/${encodeURIComponent(uhrpUrl)}` + generateURIQuery({ ...queryParams, _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
    ...
    public static GetFileInfo(uhrpUrl: string): RequestParams<BsvStorageInfoResponse, CommonErrorHandler> {
        return {
            method: "GET",
            url: getApiUrl(`/api/v1/storage/info/${encodeURIComponent(uhrpUrl)}` + generateURIQuery({ _time: Date.now() })),
            handleError: (err, handler) => {
                new RequestErrorHandler()
                    .add(500, "*", "serverError" in handler ? handler.serverError : handler.temporalError)
                    .add("*", "*", "networkError" in handler ? handler.networkError : handler.temporalError)
